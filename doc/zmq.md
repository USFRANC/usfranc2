...
The Usfranc Core daemon can be configured to act as a trusted "border
router", implementing the usfranc wire protocol and relay, making
...
The ZeroMQ feature in Usfranc Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building usfrancd:
...
For instance:
    ...
    $ usfrancd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/usfrancd.tx.raw \
...
These options can also be provided in usfranc.conf.
...
From the perspective of usfrancd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into usfrancd directly. Furthermore, no information is
...
using. Usfrancd appends an up-counting sequence number to each
